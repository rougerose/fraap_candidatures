...
function formulaires_editer_candidature_verifier_dist($id_candidature = 'new', $id_auteur = '', $retour = '') {
	$erreurs = formulaires_editer_objet_verifier('candidature', $id_candidature, [
...
function formulaires_editer_candidature_traiter_dist($id_candidature = 'new', $id_auteur = '', $retour = '') {
	$res = [];
	$row = [];
	$res = formulaires_editer_objet_traiter('candidature', $id_candidature, '', '', $retour, '', $row, '');

	if ($res['message_ok']) {
		if (intval($id_candidature) == 0) {
			$res['message_ok'] = _T('fraap_candidatures:message_candidature_enregistree');
			$res['editable'] = false;
		} else {
			$res['message_ok'] = _T('fraap_candidatures:message_candidature_modifiee');
			$res['editable'] = true;
		}
