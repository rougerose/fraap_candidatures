...
function formulaires_stages_inscription_traiter_dist($mode, $focus, $id=0) {
  ...
  $desc = $inscrire_visiteur($mode, $mail_complet, $nom, $prenom, $activite, $pass, array('id' => $id));
  ...
  // $desc = $f($mode, $mail_complet, $nom, $prenom, $pass, $activite, $id);
	...
	if (is_string($desc)) {
		...
		return array('message_ok' => _T('fraap_candidatures:identifiant_mail'),'id_auteur' => $desc['id_auteur']);
	}
}
